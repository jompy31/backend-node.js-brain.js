...
const {UserSerachs, UserSearchs} = require('../models/search')
...
router.get('/SearchData', auth, async(req,res) =>{
    ...
    .send('No se encuentra informacion.');

    let ProductID = usData[0]['data']._id;

    //ml start here
    //get the last 3 items on the usersearchs list re to our user selected
    ...
    if(user_search.length > 0) {
        for(let index = 0; index < user_search.length;index++){
            ...
            bravProducts.push(clist.data['name'])
        }
    }
    ...
    for(let index = 0; index < trainingData.length; index++){
        ...
        const net = new brain.recurrent.LSTIM();
        net.train(element, {
            iterations:100, 
            errorThresh: 0.0011
        });
                ...
                for(let index = 0; index < newAllProject.length; index++){
                    ...
                    if(FinalResultWithPrice.length < 3 ) {
                        let machineL = new ML_Result({
                            Selected_ML_Result:[0]._id,
                            UserID_ML_Result: userID
                        });
                        if(FinalResultWithPrice.length == 2){
                            console.log('Final ResultX', FinalResultWithPrice)
                            break;
                        }
                    }

                };
                // console.log('fl',FinalResultWithPrice)
                break;
            }
        }
...
router.patch('/ml_products/:ProdID', auth, async(req,res) =>{
    ...
    console.log('data', brvProducts)<KeyboardEvent
...
router.patch('/ml_cart/:ProdID', auth, async (req, res) => {
    ...
    // if already in list then we have to remove it
    let userID = req.user._id;
    let ProductID = req.params.ProdID;
...
////////////////////////// ml start here ////////////////////////////
 //get the last 3 items on the ml_cart list {our user selected}
    ...
    for(let prodID in allProducts){
        ...
        const data = allProducts[prodID];
        if(String(allProducts[prodID]._id) !== StorageManager(ProductSelected._id)){
            trainingData.push({
                input:[data.name, data.desc],
                output:String(data.name)
            });
